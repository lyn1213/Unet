...
namespace VRTK
{
    ...
    public class VRTK_AvatarHandController : MonoBehaviour
    {
        ...
        public void ChangeHandBtn()
        {
            MirrorHand();
            Transform modelTransform = (handModel != null ? handModel : transform.Find("Model"));
            Debug.Log(modelTransform.parent.localEulerAngles);           
            if (modelTransform != null)
            {
                //modelTransform.rotation = new Quaternion(modelTransform.rotation.x, modelTransform.rotation.y, modelTransform.rotation.z * -1f, modelTransform.rotation.w);
                modelTransform.parent.localEulerAngles = new Vector3(modelTransform.parent.localEulerAngles.x, modelTransform.parent.localEulerAngles.y, modelTransform.parent.localEulerAngles.z * -1f);
                // Debug.Log(modelTransform.parent.name);)
                Debug.Log(modelTransform.parent.localEulerAngles);
            }
        }
